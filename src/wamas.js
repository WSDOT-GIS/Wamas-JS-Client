/// <reference path="wamas-service.d.ts" />
...
(function (factory) {
    ...
    var DOMParser = typeof window === "undefined" ? require("xmldom").DOMParser : window.DOMParser;
    var d = require("xmldom");
    ...
    */
    var AddressCorrectionInput = (function () {
        ...
        AddressCorrectionInput.prototype.toQueryString = function () {
            return createQueryString(this);
        };
        return AddressCorrectionInput;
    }());
    /** @class
    ...
    var GeocodeInput = (function () {
        ...
        GeocodeInput.prototype.toQueryString = function () {
            return createQueryString(this);
        };
        return GeocodeInput;
    }());
    ...
    var Wamas = (function () {
        function Wamas(url) {
            if (url === void 0) { url = "http://state-wamas.wa.gov"; }
            this.url = url;
            // Trim trailing space from url if there is one.
            this.url = url.replace(/\/$/, "");
        }
        Object.defineProperty(Wamas.prototype, "addressCorrectionUrl", {
            get: function () {
                return [this.url, "addresscorrection/service.asmx/Getstandardizedaddress"].join("/");
            },
            enumerable: true,
            configurable: true
        });
        Object.defineProperty(Wamas.prototype, "geocodeUrl", {
            get: function () {
                return [this.url, "geocoder/service.asmx/FindAddress"].join("/");
            },
            enumerable: true,
            configurable: true
        });
         ...
         */
        Wamas.prototype.correctAddress = function (input) {
         ...
         */
        Wamas.prototype.geocode = function (input) {
    ...
    function xmlToObject(xml) {
        ...
        var inputRe = /input_(\w+)/;
        var output = {
            input: {}
        };
        if (xml.createTreeWalker) {
            var treeWalker = xml.createTreeWalker(xml.firstChild, NodeFilter.SHOW_ELEMENT);
            while (treeWalker.nextNode()) {
                var name_1 = treeWalker.currentNode.nodeName;
                var value = treeWalker.currentNode.textContent;
                var inputMatch = name_1.match(inputRe);
                if (name_1 === "Av_date" || name_1 === "MDexpires") {
                    value = new Date(value);
                }
                else if (numericalProperty.test(name_1)) {
                    value = Number(value);
                }
                else if (value === "") {
                    value = null;
                }
                else if (name_1 === "Results") {
                    resultCodes = {};
                    value.split(",").forEach(getResultCode);
                    value = resultCodes;
                }
                else if (name_1 === "Found" || name_1 === "MailableAddress") {
            ...
            Array.from(xml.lastChild.childNodes, function (node) {
                ...
                if (name) {
                    ...
                    var inputMatch = name.match(inputRe);
                    if (name === "Av_date") {
                        value = new Date(value);
                    }
