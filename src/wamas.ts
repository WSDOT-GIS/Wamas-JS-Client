...
class AddressCorrectionInput {
    ...
    city: string;
    Consumer: string;
    constructor(input: AddressCorrectionInputInterface) {
...
class GeocodeInput {
    ...
    zip4: string;
    Consumer: string;
    constructor(input: GeocodeInputInterface) {
        this.address = input.address || null;
        this.city = input.city || null;
        this.zip = input.zip || null;
        this.zip4 = input.zip4 || null;
...
function xmlToObject(xml) {
    ...
    let numericalProperty = /^((longitude)|(latitude)|(score))$/;
    let inputRe = /input_(\w+)/;
    let output = {
        input: {}
    };
    if (xml.createTreeWalker) { // browser
        let treeWalker = xml.createTreeWalker(xml.firstChild, NodeFilter.SHOW_ELEMENT);
        while (treeWalker.nextNode()) {
            let name = treeWalker.currentNode.nodeName;
            let value = treeWalker.currentNode.textContent;
            let inputMatch = name.match(inputRe);
        ...
        Array.from(xml.lastChild.childNodes, function (node: any) {
